...
training_data.append({"class":"view", "sentence":"display"})
training_data.append({"class":"view", "sentence":"show me the list of courses"})
training_data.append({"class":"view", "sentence":"show me the list of programs"})
training_data.append({"class":"view", "sentence":"show me the list"})
...
training_data.append({"class":"view", "sentence":"show"})
...
training_data.append({"class":"restart", "sentence":"i want to do another search"})
training_data.append({"class":"restart", "sentence":"i want to check for another course"})
...
def replaceBlanksByNone(entity):
    localEntity = entity
...
def findCourses(inpString):
    ...
    for courseRowData in courseRows:
        ...
        course["City"] = courseRowData[3]
        course["CountryName"] = courseRowData[4]
        course["CcyCode"] = courseRowData[5]
        course["FeeAmount"] = courseRowData[6]
        course["StartDate"] = courseRowData[7]
        course["Rank"] = courseRowData[8]
        course["IELTSScore"] = courseRowData[9]
        course["Duration"] = courseRowData[10]
...
def printCourses():
    global bot_name
    global entity
    courses = findCourses('') # Send blank string since no new text to be parsed
    print(bot_name, ": Found below courses matching your search:")
    print("-----------------------------------------------------------------------------")
    for course in courses:
        print(course["CourseId"],end="\t")
        print(course["CourseName"],end="\t")
        print(course["Program"],end="\t")
        if entity.showcity == True:
            print(course["City"],end="\t")
        if entity.showcountry == True:
            print(course["CountryName"],end="\t")
        if entity.showfees == True:
            print(course["CcyCode"],end="\t")
            print(course["FeeAmount"],end="\t")
        if entity.showdate == True:
            print(course["StartDate"],end="\t")
        if entity.showrank == True:
            print(course["Rank"],end="\t")
        #print(course["IELTSScore"])
        if entity.showduration == True:
            print(course["Duration"],end="\t")
        if entity.showstructure == True:
            print(course["Structure"],end="\t")
        if entity.showacadreqs == True:
            print(course["Eligibility"],end="\t")
        #print(course["URL"])
        print("-----------------------------------------------------------------------------")
...
#download_dir = "C:\\Uday\\AEGIS\\CapstoneProject\\placelist.csv"
...
#print(bot_name, ": Do you want to start with courses in ", location_city, " or ", location_country, "?")
